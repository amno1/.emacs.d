      ...
      (dolist (package packages)
        (let ((pkgdir (car (file-expand-wildcards (concat (package-name package) "*")))))
          (when (and (package-enabled package)
            ...
            (unless (member pkgdir load-path)
              (push pkgdir load-path)))))))
    ...
    (let ((loadpath (car (read-from-string
      ...
      (dolist (package (get-package-list))
        (let* ((pkdir (car (file-expand-wildcards
          ...
          (when (and pkdir
                     (file-directory-p pkdir)
                     (package-enabled package)
                     (not (package-pseudo package)))
            (verbose "Collect load-path for pkg %s" (package-name package))
            (push pkdir loadpath))))

      (push init-file-lisp-directory loadpath)
      (setf init-file-load-paths loadpath)
      (let ((init-file-target init-file-early-init-file))
        (append-to-file `(setf load-path (list ,@loadpath))))))
...
** [ vc    ] [ ] quicklisp-apropos
#+url https://github.com/mmontone/quicklisp-apropos
#+begin_src emacs-lisp

#+end_src

** [ vc    ] [ ] quicklisp-systems
#+url https://github.com/mmontone/quicklisp-systems/
#+begin_src emacs-lisp
(on-package quicklisp-systems
  (defkeys quicklisp-systems-mode-map
    "q" kill-buffer-and-window))
#+end_src
** [ vc    ] [ ] slime-breakbpoints
...
#+begin_src emacs-lisp
  ...
  (on-idle (require 'dired-setup))

  (on-hook dired-omit-mode-hook (diminish 'dired-omit-mode))
  (on-hook dired-mode-hook
    (dired-omit-mode)
    (dired-async-mode)
...
(on-hook lisp-mode-hook
  (require 'cl-extras))
(on-hook emacs-lisp-mode-hook (elisp-hooks))
