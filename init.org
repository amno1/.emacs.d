...
** Introduction
   ...
   idea with generator is to generate a machine specific setup, so no probig for
   packages, files, paths etc is needed, as well as to perform some
   ...
   This assumes that some environment variables are defined in both GNU/Linux
   and MS Windows:
   ...
   tools, bash, make & co, available, so on MS Windows mingw environemnt needs
   to be present and configured. I haven't tested with Cygwin, but I guess it
   should probably work. On MS Windows you also have to enable symlinks for
   hacked package.el to work.
...
** [x] async
#+begin_src emacs-lisp
(with-package async (async-bytecomp-package-mode 1))
