...
(defun emit-pkgs (&optional num)
  (message "Emiting packages ...")
  (save-excursion
    (let ((count 0) sxp (pkgs (get-package-list)))
      (catch 'done
        (dolist (p pkgs)
          (unless num (setq num (legnth pkgs)))
          (verbose "Checking package: %s" (package-name p))
          (when (package-enabled-p p)
            (verbose "Emiting package: %s" (package-name p))
            (goto-char (package-code-beg p))
            (ignore-errors
              (while (lte (point) (package-code-end p))
            ...
            (when (= num count)
              (throw 'done count)))))
      (message "Parsed %s packages." count))))
  ...
  (let ((index 0) srcs
    ...
    (dolist (src srcs)
      (unless (or (string-match-p src "-pkg\\.el")
        ...
        (puthash index src init-file--loadpaths-map)
