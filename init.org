...
;; Install packages
(defun package-vc-installed-p (package-name)
  (car (file-expand-wildcards (concat package-user-dir "/" package-name "*"))))

(defun install-package (package)
  (let ((pkg (intern-soft (package-name package)))
        (url (package-url package)))
    (unless (or (package-installed-p pkg)
                (package-vc-installed-p (package-name package)))
...
(defun org-init-update-packages ()
  ...
  (let ((package-list (get-package-list))
        (window-state (current-window-configuration)))
    (dolist (package package-activated-list)
      (let* ((new (cadr (assq package package-archive-contents)))
        ...
        ;; (when (package-url package)
        ;;   (package-vc-update new))
        (when (and to-install
                   (or (not pkg)
    ...
    (set-window-configuration window-state)))
...
#+end_src
** [ vc    ] [x] cl-setup
#+url https://github.com/amno1/cl-setup
#+begin_src emacs-lisp
(on-idle (require 'cl-setup))
...
(on-package inf-lisp (require 'cl-setup))
(on-hook lisp-mode-hook (require 'cl-setup))
...
** [ vc    ] [x] elisp-extras
#+url https://github.com/amno1/elisp-extras
#+begin_src emacs-lisp
...
(on-package cc-mode
  ...
  (defkeys c-mode-base-map
    ...
    "C-c C-c"    compile
    "C-c C-r"    compile-again
    "C-c C-k"    kill-compilation
    "C-<return>" yas-expand
    "M-<return>" helm-yas-complete))
...
(on-package init
  ...
  (defkeys global-map
    ...
    "C-z a"   helm-do-grep-ag
    "C-c ,"   scroll-down-sexp
    "C-c ."   scroll-up-sexp
    "C-c i"   scroll-up-paragraph
    "C-c k"   scroll-down-paragraph
    ;; some random stuff
    "C-h C-i" (lambda()
                (interactive)
